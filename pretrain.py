...
if __name__ == '__main__':
	...
	loss_list={'loss':[], 'loss_rgb':[], 'loss_rgb_eval': [], 'loss_depth':[], 'loss_depth_eval':[], 'loss_normal':[], 'loss_normal_eval':[], 'loss_freespace':[], 'loss_occupancy':[]}
	...
	for epoch in range(args.epochs):
		...
		for train_idx, train_data in enumerate(train_dataloader):
			...
			loss={'loss':0, 'loss_rgb':0, 'loss_rgb_eval': 0, 'loss_depth':0, 'loss_depth_eval':0, 'loss_normal':0, 'loss_normal_eval':0, 'loss_freespace':0, 'loss_occupancy':0}
			...
			if args.num_sample_points >=color.shape[2]*color.shape[3]:
				...
				p = sample_n(mask, args.num_sample_points) #(batch_size, n_points, 2)

			p=p.cuda(non_blocking=True) # (B,n_points,2)
			...
			#Forward pass
			valid_depth = torch.where(gt_depth>0)
			depth_range=[float(torch.min(gt_depth[valid_depth]))-padding,float(torch.max(gt_depth[valid_depth]))+padding]
			...
			calculate_photoconsistency_loss(lambda_rgb, mask_rgb, rgb_pred, color, p, 'sum', loss, eval_mode=False)
			...
			if epoch%1==0 and train_idx%1==0 or (epoch==0 and train_idx==0):
				print('epoch: %d, iter: %d, rgb loss: %2.4f, depth loss: %2.4f, normal loss: %2.4f, freespace loss: %2.4f, occupancy loss: %2.4f, total loss: %2.4f'%(epoch, train_idx, loss['loss_rgb'],
			...
			if epoch%10==0 and train_idx%1==0 or (epoch==0 and train_idx==0):
				with torch.no_grad():
					#Save a ground truth point cloud for reference
					X,_ = depth_to_3D(gt_depth, K, R, C, scaling, origin, 1)
					visualize_3D_masked(X[0].cpu().numpy(), mask[0,0].cpu().numpy(),fname=os.path.join(args.output, 'pretrain', 'meshes','gt_epoch_%04d_iter_%04d.ply'%(epoch, train_idx)))
					generate_mesh(net,device,color,X,mask,fname=os.path.join(args.output, 'pretrain', 'meshes','pred_epoch_%04d_iter_%04d'%(epoch, train_idx)))
